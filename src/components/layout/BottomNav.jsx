...
export const BottomNav = ({ theme, setIsMobileMenuOpen }) => {
    ...
    return (
        <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-40 bg-gray-900/95 backdrop-blur-md border-t border-white/10 pb-safe pt-2 px-6">
                ...
                {navItems.map((item) => {
                    ...
                    return (
                        <Link 
                            key={item.name} 
                            to={item.path}
                            className={`flex flex-col items-center justify-center w-16 h-full transition-colors ${active ? activeColor : 'text-gray-400 hover:text-white'}`}
                        >
