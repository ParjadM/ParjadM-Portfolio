...
describe('LocalizedLink', () => {
    it('prefixes paths with /fr when language is french', async () => {
        await renderLink('/fr/projects', 'fr', '/blog');
        expect(screen.getByRole('link')).toHaveAttribute('href', '/fr/blog');
    });

    it('leaves paths unprefixed for english', async () => {
        await renderLink('/projects', 'en', '/blog');
